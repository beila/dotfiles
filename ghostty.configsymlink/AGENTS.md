...
- `gtk-titlebar = false` + `window-show-tab-bar = never` — no decorations and no tab bar. Tiled windows get a compact top-right title tag from xmonad (`TopRightTag`, see `xwindow/AGENTS.md`); xmonad's `floatTags` supplies the matching tag for floating scratchpads. The always-on tab bar cost ~46px of full width with a close button, and GTK4 CSS has no `max-width` so `gtk-custom-css` cannot narrow it. Both scratchpads launch `zmx-select`, which writes `zmx` or `[<session>]` into the window title for the tag. Config changes need `ctrl+shift+,` (`reload_config`) or a fresh instance; a running ghostty keeps its loaded config until the process is killed.
