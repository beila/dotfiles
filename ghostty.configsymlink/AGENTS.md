...
- `gtk-titlebar = false` + `window-show-tab-bar = never` — no decorations and no tab bar. Tiled windows get a compact top-right title tag from xmonad instead (`TopRightTag`, see `xwindow/AGENTS.md`); the always-on tab bar cost ~46px of full width with a close button, and GTK4 CSS has no `max-width` so `gtk-custom-css` cannot narrow it. The two ghostty **scratchpads are floating windows**, which xmonad decorations never reach, so `xmonad.hs` launches those with `--window-show-tab-bar=always` — per-window CLI overrides work because each scratchpad is its own instance. Config changes need `ctrl+shift+,` (`reload_config`) or a fresh instance; a running ghostty keeps its loaded config.
