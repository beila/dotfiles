...
- Sync scripts: `~/.dotfiles/script/sync_all` (all jj/git repos via plocate, triggered by `sync-repos.timer`), `sync_repo` (single repo), `script/test_sync_repo.sh` (divergence/merge/timeout test harness)
  - `sync_all` iterates `.jj`/`.git` markers under `$HOME` from plocate, filters noise paths (`.cache`, `.cargo`, `.nix-profile`, `node_modules`), and **deduplicates by `jj root` / `git top-level`** so monorepos with many submodule markers trigger sync_repo once per underlying repo root (not once per marker). Logs via `script/logger/log.sh` with tag `sync_all`: INFO lines for START + discovery count; ERROR summary + non-zero exit when any per-repo sync fails. Workspaces of the same repo are still iterated separately (each has its own `jj root`), which is intentional — each workspace has its own `@` to sync. Test harness: `script/test_sync_all.sh` (25 assertions; fake plocate / sync_repo / jj / git).
  - `sync_repo` jj path: per-repo `flock`; exits early if no `backup` remote (checks both `jj git remote list` and falls back to `git remote get-url backup`; additional defensive check for empty `BACKUP_URL`); `jj git root` falls back to `git rev-parse --git-dir` when jj's view is degraded (stale op log). Single `jj log -r @` call snapshots atomically; runs `jj new` on non-empty OR empty-merge `@`; describes with AI commit message (via `commit-msg` with `VERBOSE=1`); uses `if(description, ...)` for empty description check (jj has no `description.is_empty()`). `LOG_CONTEXT` derived from path-relative-to-home (not basename) so different workspaces/worktrees of the same-named repo don't collide in log filenames (e.g. `~/dev/pro/src/IgnitionX` → `pro-src-IgnitionX`, `~/dev/pro2/src/IgnitionX` → `pro2-src-IgnitionX`).
  - Auto-merge bookmark: reads `sync.bookmark` from per-repo jj config (`jj config set --repo sync.bookmark NAME`); skips fetch/merge/rebase when not set
  - Workspace name: matched by current `@` commit_id against `jj workspace list`
  - Auto-merge (Step 2): unified merge-based flow — always does `jj new --no-edit LOCAL_BM_REV REMOTE_REV -m "Merge XXX into <bm> from <hostname>"` (with `--no-edit` so `@` stays on the user's working-copy commit), then `jj simplify-parents` to collapse redundant parents, then `jj rebase -r @ -d <merge>` so `@` hops on top. `jj simplify-parents` is the single discriminator for all sub-cases: `empty+non-merge` after simplify means degenerate (equal/ahead/ff); otherwise it's a genuine 2-parent merge. Degenerate path: abandon the empty wrapper (which auto-rebases `@` onto the survivor), set bookmark to the survivor, push only if survivor==LOCAL_BM_REV (we advanced remote). Merge path: re-describe via `bin/commit-msg` (AI-generated summary) BEFORE push so the remote receives the final form, then set bookmark and push. Conflicts in the merge are reported with filenames (`jj resolve --list`) and the merge is abandoned — bookmark stays put, next sync retries after user resolves. The new-remote case (REMOTE_REV empty) is the only one kept as a separate push path since `jj new --no-edit L ""` doesn't work.
  - Ambiguous-bookmark handling: resolves `$LOCAL_BM_REV` to a commit_id BEFORE fetch so post-fetch revsets don't fail when the bare bookmark name becomes conflicted (local vs remote). `jj new --no-edit` is checked for success and logs `MERGE-FAIL` (CRITICAL, notified) instead of silently falling through. The new merge commit is located via `latest(L+ & R+)` (children of both parents, newest); using `description(exact:...)` would require escaping jj's trailing-newline semantics.
  ...
  - Event logging via `script/logger/log.sh` (level per event tag): `FETCH-OK`, `PUSH-OK`, `FAST-FORWARD`, `SKIP`, `NO-BACKUP-URL`, `START` at INFO; `NETWORK-ERR`, `TIMEOUT`, `BENIGN-DEL` (remote ref doesn't exist) at WARN/DEBUG (transient, not notified); `OTHER-ERR`, `MERGE-CONFLICT` at ERROR (notified); `MERGE-FAIL` at CRITICAL (notified). `classify_cmd` wraps each network call and routes failures to NETWORK-ERR / OTHER-ERR / BENIGN-DEL based on stderr pattern matching.
...
- Private dotfiles: `~/.dotfiles/private-dotfiles/` — gitignored colocated jj/git repo; added as `git+file://` flake input; stores machine-specific config (host configs, tokens, webhook URLs), Brazil JDK setup, and per-host `homeConfigurations`; zsh `**/*.zsh` glob auto-sources; `install.sh` files run by `script/install`; `ssh.filesymlink/` provides SSH host aliases; `telegram.env` holds `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` for the notify-webhook Telegram backend; `logger.nix` writes `~/.config/environment.d/20-logger.conf` so systemd-launched jobs inherit `LOG_ROOT`/`LOG_REL_BASE`. See `private-dotfiles/AGENTS.md` for Brazil, flake input details, and how to add a new host
...
- Battery notify: `~/.dotfiles/script/battery-notify` — systemd timer every 1min. Uses `log.sh`; logs only at threshold transitions (ERROR for missing battery, WARN for critical ≤10%, INFO for low ≤20%). At 20% fires `notify-send` (normal urgency). At 10% calls `battery-osd` instead — a huge pseudo-transparent overlay that's hard to miss. `BAT`, `POWER_SUPPLY_DIR`, `STATE_FILE`, `BATTERY_OSD_BIN` env-overridable for tests. Test harness: `script/test_battery-notify.sh` (43 assertions; fake sysfs + stubbed notify-send and battery-osd; sets `LOG_KEEP_THRESHOLD=DEBUG` so INFO/WARN log lines persist for assertions).
- Battery OSD: `~/.dotfiles/xwindow/bin/battery-osd.py` — Python source for the OSD; built as a single wrapper binary `battery-osd` via `pkgs.writers.writePython3Bin` in `home.nix`, bundling pycairo + python-xlib so no full Python ends up in `$PATH` (would conflict with awscli2's bundled python3). Renders huge "󰂃 X%" with cairo (red fill, thick black stroke, drop shadow), then displays in an override-redirect X window whose XShape mask is derived from the rendered alpha (red fill, thick black stroke, drop shadow) — the "background" is genuinely transparent (XShape clips), so the OSD is highly visible without painting a big colored block. Works without a compositor. On multi-monitor setups, enumerates active CRTCs via Xrandr and shows one window per monitor sized to that monitor (de-duped so mirrored displays don't double-render); falls back to the whole-screen rect if RandR is unavailable. Picks largest font that fits 80% screen width × 35% height. Splits cairo→X `PutImage` calls into row chunks because python-xlib doesn't use BIG-REQUESTS for these ops (16-bit length cap → ~256 KB per request). `--render-png` flag renders to a PNG without touching X (used in tests / previews). `--duration N` sets visible seconds (default 10). Catches SIGTERM/SIGINT for clean window teardown.
- Random lockscreen: `~/.dotfiles/xwindow/bin/random-lockscreen` — daily systemd timer. Uses `log.sh`; WALLPAPER_DIR env-overridable (default `~/Pictures/Favourites`); actionable ERRORs for missing dir, empty dir, DBus/schema unreachable, gsettings missing. Test harness: `script/test_random-lockscreen.sh` (23 assertions + 1 skip when real gsettings on PATH; fake wallpaper dir + stubbed gsettings).
