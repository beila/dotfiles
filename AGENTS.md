...
| `bin/` | `logrun`, `commit-msg`, `say`/`say-en`/`say-ko` TTS, Claude Code hooks, `vpn-up`/`vpn-watch`, `zellij-cycle`, `zmx-select`, `notify-webhook`, `mcp-tts`, `evolution-reauth` (in-place M365 token refresh, no mail re-download). |
