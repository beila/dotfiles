...
- xmonad config: `~/.dotfiles/xwindow/xmonad.symlink/xmonad.hs` (symlinked to ~/.xmonad/)
  ...
  - `stripZoomFullscreenHook`: forces Zoom "Meeting" windows to stay tiled. Zoom creates the window titled "Zoom Workplace" then renames to "Meeting" after the ManageHook has run, so `title =? "Meeting"` can't match there. The event hook watches PropertyNotify on `_NET_WM_STATE`, `_NET_WM_NAME`, and `WM_NAME`; when a zoom window becomes titled "Meeting", it strips `_NET_WM_STATE_FULLSCREEN` from `_NET_WM_STATE` and re-sinks via `W.sink`. Also paired with `setEwmhFullscreenHooks`: fullscreen hook is `composeOne` returning `idHook` for zoom+Meeting (default `doFullFloat` otherwise), preventing `ewmhFullscreen` from floating Meeting on ClientMessage fullscreen requests; unfullscreen hook unchanged (`doSink`). Other Zoom windows (`zoom_linux_float_message_reminder`, `zoom_linux_float_video_window`) unaffected
...
- Sync scripts: `~/.dotfiles/script/sync_all` (all jj/git repos via plocate, triggered by `sync-repos.timer`), `sync_repo` (single repo), `jj_snapshot_all` (snapshot all jj repos via plocate)
  ...
  - Workspace snapshot bookmark (Step 0): creates `hostname/workspace` bookmark at `PUSH_REV`, force-pushes via raw git delete+push, then deletes local bookmark — captures current workspace state per workspace even without a named bookmark
  - Requirements documented as comments in script: (1) commit with AI message if non-empty, (2) force-push commit as `hostname/workspace_name`, (3) push all prefixed bookmarks, (4) safely merge and push tracked bookmark
