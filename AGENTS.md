...
- [ ] automatically use logrun in long-duration or long-output commands
  ...
  - **Latency budget**: short commands must add ≤20ms vs bare. `zsh -ic` adds ~800ms (full zshrc replay), so the widget must avoid it for the prompt path:
    ...
    - **Functions**: opt-in via `LOGRUN_AUTO_FUNCTIONS` (defaults empty). Functions in the list go through `logrun --auto -c "$BUFFER"` (slow path, `zsh -ic`). Functions NOT in the list run unwrapped via normal zle dispatch. This matches the user's reality — most functions (`l`, `c`, `p`, `o`, jj wrappers like `ju`/`jda`/`jj`, git helpers) never run long; only the wrapper-style functions that shell out to long-running tools (`j n ji ni jr njr nijr sync-rsync sync-ssh docker_here*`) are worth logging.
