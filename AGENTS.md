...
- [x] random/numbered voices and speed controls for `say-en`/`say-ko`/`say-es` — shared `bin/say-voice.sh` maps caller→voice via `voice = pool[sha256(key) mod N]` when `--voice N` is omitted; explicit numbers are one-based. Key = `$SAY_VOICE_KEY` if set (empty ⇒ unidentified ⇒ default voice; `"1"` is still an opaque hashed key), else `$PPID`. All three backends accept `--voice`, `--speed`, and `--list-voices`; Edge uses rate percentages and Piper translates the percentage to inverse length scale. Spanish = 7 Edge voices across Spain/Mexico/US, Korean = 3 Edge voices, English = 5 Piper voices with lazy per-voice download (alba fallback). `say` still auto-routes only Hangul versus non-Hangul; use `say-es` directly. See `bin/AGENTS.md`. Test: `bin/test_say_voice.sh`.
