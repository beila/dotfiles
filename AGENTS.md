...
- Battery notify: `~/.dotfiles/script/battery-notify` — systemd timer every 1min. Uses `log.sh`; logs only at threshold transitions (ERROR for missing battery, WARN for critical ≤10%, INFO for low ≤20%). `BAT`, `POWER_SUPPLY_DIR`, `STATE_FILE` env-overridable for tests. Test harness: `script/test_battery-notify.sh` (33 assertions; fake sysfs + stubbed notify-send).
