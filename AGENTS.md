...
- Battery notify: `~/.dotfiles/script/battery-notify` — systemd timer every 1min. Uses `log.sh`; logs only at threshold transitions (ERROR for missing battery, WARN for critical ≤10%, INFO for low ≤20%). At ≤20% (once per discharge cycle) fires `notify-send` (normal urgency). At ≤10% calls `battery-osd` (huge pseudo-transparent overlay that's hard to miss) and re-fires on every percent change while still ≤10% so the user keeps noticing as the situation worsens — state file stores `crit:<capacity>` so the same capacity doesn't double-fire but a drop to a lower percent does. Charging/full/unknown clears the state. Env-overridable for tests: `BATTERY_NOTIFY_BAT_DIR`, `BATTERY_NOTIFY_POWER_SUPPLY_DIR`, `BATTERY_NOTIFY_STATE_FILE`, `BATTERY_OSD_BIN`. Test harness: `script/test_battery-notify.sh` (47 assertions; fake sysfs + stubbed notify-send and battery-osd; sets `LOG_KEEP_THRESHOLD=DEBUG` so INFO/WARN log lines persist for assertions).
