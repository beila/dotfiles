...
**Env propagation into the floating pane**: only an explicit allowlist (`TERM`, `PATH`, `FZF_ZELLIJ`, `FZF_DEFAULT_OPTS`, `FZF_DEFAULT_COMMAND`, `BAT_THEME`) is forwarded. `FZF_DEFAULT_COMMAND` is mandatory — built-in `fzf-cd-widget` / `fzf-file-widget` set it to `FZF_ALT_C_COMMAND` / `FZF_CTRL_T_COMMAND` before invoking `__fzfcmd`, and without forwarding fzf falls back to its `--walker` and lists cwd subdirectories instead of our zoxide+dirs+worktree merged list.
