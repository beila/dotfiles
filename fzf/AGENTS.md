...
**Env propagation into the floating pane** is an explicit allowlist (`TERM`, `PATH`, `FZF_ZELLIJ`, `FZF_DEFAULT_OPTS`, `FZF_DEFAULT_COMMAND`, `BAT_THEME`, `JJ_FZF_OPERATION`). `FZF_DEFAULT_COMMAND` is load-bearing: built-in widgets (`fzf-cd-widget` / `fzf-file-widget`) set it from `FZF_ALT_C_COMMAND` / `FZF_CTRL_T_COMMAND` before invoking `__fzfcmd`; dropping it makes fzf fall back to its `--walker` and list cwd subdirectories. `JJ_FZF_OPERATION` keeps jj previews on the operation captured before the floating pane opened.
...
- **`functions.sh`** — jj-first / git-fallback functions. `fzf_down()` passes `--no-height`, so all custom pickers fill the terminal outside zellij regardless of whether the shell is in a zmx session; zellij still supplies its own adaptive floating-pane viewport. Each `_g*` dispatcher delegates to `_j*` (jj) or `_git_*` (git). For jj repos, `_gh` starts the wider `_jhh` view directly when the effective `sync.remote-bookmark` config key exists; otherwise it starts `_jh`. The probe reuses the picker's pinned operation. `_jb`/`_jt` previews use `unique_boundary()` revset (see `jj.configsymlink/AGENTS.md`). `_jb` parses `jj bookmark list` output directly (indented remote-tracking lines like `  @hj …` get re-prefixed with the parent bookmark via awk so the row says `nix@hj …`). Toggles via `become`: `_jh`↔`_jhh` (ctrl-h), `_jb`↔`_jbb` (workspaces, ctrl-b), `_jb`↔`_jbr` (remote bookmarks via `--all-remotes`, ctrl-r; header checkbox `☐/☑ remotes` shows the state), `_jy`↔`_jyy` (op log, ctrl-y). `_jh` reads `jj config get revsets.log` at runtime for its revset, so it shows exactly what a bare `jj log` shows in that repo — including repos that override `[revsets].log` in their own config (hardcoding `log()` diverged in such repos); `_jhh` widens to `::workspace_view()` for the full ancestry. See `jj.configsymlink/AGENTS.md`. ctrl-o inserts empty revision after selected (`jj new --no-edit --after`), uses `transform:` colon form for error display. fzf query preserved across toggles via `{q}`→`--query`. **ctrl-s** (in `_jh`/`_jhh`) toggles a `jj log -s`-style file view by `reload`ing with the `fzf_files_suffix` template appended (`_jj_log_reload` builds the command). **ctrl-x** (in `_jh`/`_jhh`) yanks the **git commit id** instead of the change id: `become(printf '%s\n' {+4})` outputs hidden tab field 4 (`commit_id.short()`, 12-char hash; `{+4}` so multi-select yields one per row, `--ansi` strips its colour) straight to the widget's stdout — same exit path as the ctrl-h toggle's `become`. Default Enter still yields the change id via `--accept-nth=2`. All log lines carry three **hidden tab fields** (`<display>\t<change-id>\t<path>\t<commit-id>`; see jj `config.toml`): `_jj_log_fzf` runs with `--delimiter='\t' --with-nth=1` so only the visible column shows/searches, `--accept-nth=2` and `_jj_change_id`/`_jj_extract_id` (`cut -s -f2`) read the change id, `_jj_commit_field` (`{+4}`) reads the commit id, and the preview reads the path from field 3 (`_jj_extract_path`, `cut -s -f3`) to diff just that file — commit lines (empty path) instead get `jj show --summary` + full `jj diff`. The tab columns are immune to the graph area's variable spaces, fixing both wrong extraction on file lines and the merge/elision-space bug. The `cut -s` (only-delimited) is essential: graph-only connector rows (`│ │`, `~`) carry no tab, and plain `cut -f2` would echo the whole row → the preview would run `jj -r "│ │"` → "Failed to parse revset"; `-s` makes those rows extract empty so the preview's `[ -z "$id" ] && exit 0` guard suppresses them. File names are coloured like native `jj log -s`; no id is shown on file lines. **Preview** (`_jj_log_preview`) branches three ways on the focused line's hidden fields + `$FZF_PROMPT`: files-view-off commit line → `jj show --summary` (with file list) + full diff; files-view-on commit line → header+desc only (`jj log -T builtin_log_detailed`, **no** file list since the names are already in the fzf list); files-view-on file line → header+desc + only that file's `jj diff -- <path>`. **Alignment**: jj's graph indents a commit's first continuation line one space less than the rest, so `_jj_log_reload` pipes file-view output through `_jj_align_files` = `gawk -f functions.sh/jj-align-files.awk` (gensub normalizes the gap after the last `│` to 3 spaces on file lines only); a template can't fix this. The gawk program MUST live in a file, not inline: the reload command crosses several shell hops (fzf transform → `echo` → fzf → `sh -c`), and an inline program's `$1`/`$3` get expanded away by those shells, corrupting it to `NF>=3 && != { = gensub(...) }` → "Command failed". A bare `-f <path>` has no `$` to eat. Toggle state lives in the prompt (`log> ` ↔ `log+files> `, the same prompt-as-state trick as `_file_browse`), which `ctrl-o` reads back so an insert keeps the current view; state does NOT survive the ctrl-h `become` swap (new process), same as the existing query-only handoff. Line-number focus uses `result:pos(N+1)+unbind(result)`. `fzf_down()` deliberately adds **no** `ctrl-/` binding — the layout-aware override lives in `fzf-zellij` so it can also detect `--reverse` injected via `FZF_DEFAULT_OPTS` env (e.g. by built-in `fzf-cd-widget` / `fzf-file-widget`).

**Path coordinates:** file-producing pickers use paths relative to their launch directory, and `fzf-zellij` preserves that directory with `zellij run --cwd "$PWD"`.
The jj log template follows the same rule by placing `f.path().display()` in both the visible file row and hidden preview field.
This keeps `_gh` and `_ghh` file previews valid below the workspace root without changing the selected change ID.

**Repository-load optimization:** Every jj picker first runs one serialized `jj operation log --limit 1` command.
It performs the workspace snapshot and returns its full operation ID, which is locally exported as `$JJ_FZF_OPERATION` for the picker lifetime.
The initial list producer, read-only follow-up commands, preview shells, and `become` toggle targets use `JJ_SERIALIZED_READ_ONLY=1 jj --at-operation "$JJ_FZF_OPERATION"`.
These commands share the exact repository view even if another process advances the current operation, and they bypass the external `jj-serialized` sync lock.
The bypass prevents a deadlock in fzf.
Without it, an initial producer can fill the pipe while holding the lock, and then a reload waits for that lock while fzf stops draining the producer.
The bypass is safe because `--at-operation` implies `--ignore-working-copy`, and these commands cannot snapshot or mutate the working copy.
The initial snapshot and mutating commands such as `jj new` remain serialized.
`jj new` also remains unpinned because mutating at an older operation creates concurrent history; its list reload stays on the picker's original snapshot.
The normal conceptual `jj show --summary` plus `jj diff` preview is produced by one `jj show`, whose template appends `diff.summary()` before the normal full patch.
This matters in `.dotfiles`, where every repository open costs about 0.3 seconds because `.jj/repo/store/extra` contains roughly 2.3 million files.

`fzf_down()` passes `--no-mouse` for every custom picker in `functions.sh`, including all jj/Git leaves and `become` toggle targets. This lets ordinary terminal dragging select preview text; keyboard preview scrolling remains available.

- **`test_toggle_query.sh`** — non-interactive test for toggle query/focus preservation, ctrl-o binding, change-ID extraction, the ctrl-x commit-id yank (binding presence + real-fzf field-4 extraction via `--accept-nth=4` proxy), and the `ctrl-/` preview-cycle behaviour (incl. `fzf-zellij`'s `--reverse` detection via stubbed-fzf-on-PATH + `FZF_ZELLIJ=1` plain-fzf fallback path); run with `zsh fzf/functions.sh/test_toggle_query.sh`.
