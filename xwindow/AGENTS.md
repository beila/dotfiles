...
- **Polling, not file-watching**: `GLib.timeout_add_seconds(30, …)` polls the status word every 30 s, evaluating once at startup. Polling is deliberate — crossing the expiry timestamp must flip the OSD on even when the cookie file has not changed, which a file-change watch would miss. The 30 s cadence matches the state contract.
...
- **OSD stuck on / off after auth change**: the flip is bounded by the 30 s poll — wait one cycle. `journalctl --user -u midway-osd` shows child stderr (`midway-osd[child]: …`) if rendering failed on a monitor.
