...
Pure red `#FF0000` at exactly `0.5` alpha, `MW`, JejuHallasan, **no outline, no shadow**. Same physical **70 mm height** as the `한` box; **110 mm wide** so the two Latin letters never clip (the one-glyph `한` box is 60 mm). Seated immediately to the **left** of the `한` box with a fixed **6 mm** physical gap and no overlap.

- **Adjacency is derived, not approximated**: the MW style shares the `한` box's right-edge inset (`HANGUL_SLOT_OFFSET_X_FRAC`) and top offset (`HANGUL_SLOT_OFFSET_Y_FRAC`), then steps left by `sibling_offset_mm(hangul_ref, 6.0)` = −(60 + 6) mm. Because the window widths cancel in that derivation, the gap is the same physical distance on every monitor regardless of resolution or DPI — no drift across mixed-DPI panels. See the OSD library section for `sibling_offset_mm` / `HANGUL_SLOT_*`.
- **Decorative font, proven not fallback**: JejuHallasan is missing 20 extended-Latin glyphs for fontconfig's `en` orthography (so Pango's default fontmap hides it — same reason as hangul-osd), but it DOES cover basic-latin `M` and `W`. The `use_pango=True` + `font_file` (app-font registration via `FcConfigAppFontAddFile`) path makes Pango's matcher see the family, and `test_midway_osd.py` asserts every glyph run for `MW` resolves to family `JejuHallasan`, not a generic fallback. No plain sans-serif fallback is accepted.
...
- `python3 xwindow/test_midway_osd.py` — parser-status delegation, transition/idempotency, style, geometry (1920×1200 + 4K mixed-DPI), click-through, offline PNG, and the JejuHallasan font-resolution proof. The geometry/font tests need the real `osd` library (cairo + Xlib) and, for the font proof, Pango/PangoCairo typelibs on `GI_TYPELIB_PATH` + `MIDWAY_OSD_FONT_FILE`; those tiers skip cleanly when the deps are absent.
...
- **`MW` renders as boxes / wrong font**: `MIDWAY_OSD_FONT_FILE` must point at the JejuHallasan ttf and the Pango/PangoCairo typelibs must be on `GI_TYPELIB_PATH` (both set by the wrapper). Verify with `python3 xwindow/test_midway_osd.py -k jejuhallasan`.
