...
- **`raiseFocused`** (logHook): calls `raiseWindow` on the focused window after every focus change. Picom draws a window's shadow at that window's Z-level, so without this, the glow is hidden wherever a higher-stacked tiled neighbor overlaps the shadow zone. Raising the focused window ensures its glow paints above all neighbors symmetrically. It then calls **`raiseDecorations`** — see below, since raising the client would otherwise bury the title tag painted on top of it.
- **`raiseOsdWindows`** (last in logHook): re-raises override-redirect windows whose resource name is `osd` after focused clients, decorations, and floating title tags. Persistent Hangul OSD windows otherwise become permanently covered on the first focus change after they are mapped; one-shot OSDs can be covered before their timeout for the same reason. Matching only the OSD library's resource name leaves normal menus and dropdowns untouched.

## Window title tag (`TopRightTag`)

A ~190×20 title tab pinned to the **top-right corner of each tiled ghostty window** (`layoutHook = smartBorders $ avoidStruts $ decoration shrinkText tagTheme TopRightTag $ …`). ghostty runs with `gtk-titlebar = false` and the panel has no tasklist, so the window title — which `zsh/terminal.zsh` prefixes with `[$ZMX_SESSION]` — had nowhere to show; unlike a prompt segment it survives a long-running command. Theme is the LEGO-orange accent: `#F8BB3D` text + 1px border when focused, grey text and no border otherwise.

Four things are load-bearing:

- **`shrink _ _ r = r`** — the tag is painted *over* the client's corner instead of reserving a full-width `decoHeight` strip whose left half would be empty background. Costs a few characters of row 1 rather than a whole row.
- **`decorate` (not `pureDecoration`)** — it runs in `X`, so it can `runQuery (className =? "com.mitchellh.ghostty")` and return `Nothing` for everything else. Browsers and editors keep their own titles undecorated.
- **`raiseDecorations` in the logHook** — `raiseFocused` raises the client, which buries an overlapping decoration; the tag then only showed on *unfocused* windows. The fix walks the root's children and raises anything whose **resource name** is `xmonad-decoration`. Note the resource *class* is plain `"xmonad"` — matching on class finds nothing, which is exactly the bug that made the first attempt look like a no-op.
- **Floats need their own tags** (`floatTags`, logHook). `XMonad.Layout.Decoration` only sees windows the layout placed, so the two ghostty scratchpads — floats — were never tagged, which is the workflow that wants it most once they run zmx sessions. `floatTags` keeps one `XMonad.Util.XUtils` window per visible floating ghostty, parked at the client's top-right (`getWindowAttributes` for the rect), painted via `paintAndWrite` with the same `tagTheme` colours, raised after `raiseFocused`, and destroyed when the window goes away. State lives in a non-persistent `ExtensionClass` (`FloatTags` cache keyed by client, plus a lazily-initialised `XMonadFont`) and is only repainted when the rect or title actually changed — the logHook fires on every event, and recreating windows there flickers.
- **Sizes are HiDPI-scaled and tunable from X resources** (`tagMetrics`). Bases are 96dpi-equivalent — font `4.5`, `320x9` — multiplied by `Xft.dpi / 96` read from `xrdb -query` at startup (2.0 here, giving a 9pt font in a 640x18 tag). Override without editing this file: `echo 'xmonad.tag.height: 9' | xrdb -merge && xmonad --restart`, then reset the layout; keys are `xmonad.tag.{fontSize,width,height}`. `xrdb -merge` is session-only and there is no `~/.Xresources` (Xft.dpi comes from XSETTINGS), so anything worth keeping belongs in the code defaults. Same `Xft.dpi/96` convention as the OSD scripts in `bin/`. Two traps: X reports a nominal 96dpi on HiDPI (`xdpyinfo` says 96 on a 3840x2400 screen), so the DPI must come from the resource DB; and xmonad's Xft rendering ignores `Xft.dpi` (GTK honours it), so the *font size* needs scaling too — scaling only the box leaves half-size text.
