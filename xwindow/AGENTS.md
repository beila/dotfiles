...
LEGO colour 21 "Bright Red" `#B40000` at exactly `0.5` alpha, `MW`, **Great Vibes** (ornate OFL swash script), **no outline, no shadow**. A **66 × 42 mm** box — a uniform 0.6 scale of the original 110 × 70 mm box, so its `11:7` aspect ratio is preserved exactly and its ~66 mm width sits close to the `한` slot's 60 mm without stretching either axis. **Vertically centred inside the 70 mm `한` slot** (not top-aligned), seated immediately to the **left** of the `한` box with a fixed **2 mm** physical gap and no overlap.

- **Adjacency and vertical centring are derived, not approximated**: the MW style shares the `한` box's right-edge inset (`HANGUL_SLOT_OFFSET_X_FRAC`) and top offset (`HANGUL_SLOT_OFFSET_Y_FRAC`), then steps left by `sibling_offset_mm(hangul_ref, 2.0)` = −(60 + 2) mm and down by `(70 − 42)/2` = 14 mm (`offset_y_mm`) so the shorter box is vertically centred within the 70 mm slot. Both the gap and the centring are expressed in mm, so the window widths / heights cancel in the derivation and neither drifts across mixed-DPI panels. See the OSD library section for `sibling_offset_mm` / `HANGUL_SLOT_*`.
- **Decorative swash font, proven not fallback**: Great Vibes is a genuinely ornamental OFL script face whose capitals `M`/`W` carry abundant curling strokes. The `use_pango=True` + `font_file` (app-font registration via `FcConfigAppFontAddFile`) path makes Pango's matcher resolve the `Great Vibes` family, and `test_midway_osd.py` asserts every glyph run for `MW` resolves to family `Great Vibes`, not a generic fallback. No plain sans-serif fallback is accepted.
...
- `python3 xwindow/test_midway_osd.py` — parser-status delegation, transition/idempotency, style (LEGO `#B40000` / `0.5` alpha / premultiplied-pixel check), geometry (66×42 mm height, vertical centring in the 70 mm slot, 2 mm gap; 1920×1200 + 4K mixed-DPI), click-through, offline PNG, and the Great Vibes font-resolution proof. The geometry/font tests need the real `osd` library (cairo + Xlib) and, for the font proof, Pango/PangoCairo typelibs on `GI_TYPELIB_PATH` + `MIDWAY_OSD_FONT_FILE`; those tiers skip cleanly when the deps are absent.
...
- **`MW` renders as boxes / wrong font**: `MIDWAY_OSD_FONT_FILE` must point at the Great Vibes ttf and the Pango/PangoCairo typelibs must be on `GI_TYPELIB_PATH` (both set by the wrapper). Verify with `python3 xwindow/test_midway_osd.py -k great_vibes`.
