...
- `midway-genmon` — Midway session plus AEA cookie and posture status for xfce4-genmon-plugin. It consumes `bin/midway-status`, which combines local credential expiration times, `mcscli is-valid posture`, and Midway's live `/api/session-status` result without rewriting the cookie jar. The AEA checks remain separate because valid device posture does not renew the cookie consumed by CR. The shared result is cached for 60 seconds while plugin 7 polls every 30 seconds. The always-visible panel label is just the coloured dot and `MW`; the Midway time remaining lives in the hover tooltip (`<tool>`), shown in every state where the Midway cookie is still valid — the `valid` state (`Midway remaining: 7h 32m`) and each AEA-only failure (`aea-cookie`/`aea-missing`/`aea-posture`), whose tooltips carry the same `Midway remaining` line since Midway itself is unexpired there. The indicator colour follows only the Midway session's remaining time: green at 8 hours or more, yellow from 2 to 8 hours, and red below 2 hours or for a definite Midway rejection, expiry, or absence. AEA expiry, absence, or rejected posture is reported in the tooltip without changing that colour. During an AEA-only failure the shared checker does not call the Midway API, so the colour band follows the still-future local Midway timestamp while the tooltip identifies the AEA problem. Magenta `? MW` means the network/API check was unavailable, so only the untrusted local expiration time is known. The persistent `midway-osd` daemon consumes the same checker but likewise ignores AEA-only reasons; `just mwinit` still treats them as invalid. `MIDWAY_STATUS_COMMAND` and `MIDWAY_NOW` make rendering deterministic in `bash xwindow/test_midway_genmon.sh`; the checker itself is covered by `bash bin/test_midway_status.sh`.
