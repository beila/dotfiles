...
- **Floats never get decorated.** `XMonad.Layout.Decoration` only sees windows the layout placed, and the two ghostty scratchpads are floats — so they pass `--window-show-tab-bar=always` on their own command line and use ghostty's own tab bar instead, while `ghostty.configsymlink/config` keeps `window-show-tab-bar = never` for everything else.
