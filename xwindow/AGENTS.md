...
- **Lock-screen Hangul OSD**: Hangul windows use resource name `hangul-osd`. During initialization, XMonad adds `PropertyChangeMask` to its existing root event mask so `_XMONAD_SCREEN_LOCKED` updates reach the event hooks. When that property becomes active, and after any window maps while locked, `raiseHangulOsdOnLockHook` re-raises only those windows. This handles either ordering between the lock-state signal and the lock-screen map without promoting Zoom, battery, or other generic OSDs over the lock screen. The OSD remains pointer-transparent.
...
- **Persistent topology following**: `display_on_all_monitors(..., follow_monitor_changes=True)` subscribes to RandR screen/CRTC/output changes and rebuilds every OSD window from the current active-monitor geometry. Hangul and Midway enable it so lid close/open and monitor connection changes cannot leave their persistent windows off-screen; one-shot battery/Zoom displays retain the static path.
