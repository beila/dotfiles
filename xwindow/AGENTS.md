...
A compact title tag is pinned over the **top-right corner of every visible ghostty window**. ghostty runs with `gtk-titlebar = false` and the panel has no tasklist, so the window title — which `zsh/terminal.zsh` prefixes with `[$ZMX_SESSION]` — had nowhere to show; unlike a prompt segment it survives a long-running command. The focused tag uses LEGO orange (`#F8BB3D`); inactive tags retain a visible `#C7CBD1` label and `#69717F` border on `#1d1d1d`.
...
- **Focus is part of the cache key**: the cache stores client rectangle, title, focus state, and `TagMetrics`. Omitting focus previously left a newly focused scratchpad painted with the low-contrast inactive style until its title or geometry changed.
