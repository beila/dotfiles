...
- **Follow loop drains the internal event queue before selecting (python-xlib gotcha)**: the loop must call `d.pending_events()` and process any queued events *before* blocking on `select(d.fileno())`. python-xlib buffers events in an internal queue, and the `d.sync()` inside `create_windows()`/`destroy_windows()` does a non-blocking socket read — so a RandR event arriving mid-rebuild lands in that queue and leaves the fd **not readable**. Selecting on the raw fd alone then blocks forever with the event stranded. Because one monitor change emits several events back-to-back (Output/Crtc/Screen), the event carrying the *final* geometry is the one most often swallowed, so the OSD sticks on an intermediate/old layout — observed as a single mis-placed Midway window with its child at 0% CPU blocked in `select` after a DisplayLink topology change. Fixed by `if not d.pending_events(): select(...)`.
- **Topology regression test**: `python3 xwindow/test_osd.py` covers two cases: (1) an fd-readable RandR event rebuilds windows for the current monitors, and (2) an event that is only in python-xlib's internal queue (fd never readable) still drives a rebuild — the regression guard for the stranded-event bug above. The second test fails against the pre-fix select-first loop.
