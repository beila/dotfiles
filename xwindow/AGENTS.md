...
A pre-outlined **vector SVG** asset (`xwindow/osd/assets/mw.svg` — UnifrakturCook blackletter `MW` with LEGO colour 21 "Bright Red" `#B40000` baked in) painted at exactly `0.5` alpha, **no outline, no shadow**. **No font at runtime** — the glyph is stored as paths and rasterised by librsvg at the exact per-monitor pixel size, so it stays crisp at any DPI (the font moved to asset-authoring time; see `assets/generate.py`). The box is sized to the asset's aspect ratio (≈ `92 × 42 mm`) so the vector fills it with no letterboxing, **vertically centred inside the 70 mm `한` slot** (not top-aligned), seated immediately to the **left** of the `한` box with a fixed **2 mm** physical gap and no overlap.
...
- **Vector asset, not a runtime font**: `mw.svg` is generated once from UnifrakturCook by `assets/generate.py` (glyph → paths; the generator asserts the glyph resolved to `UnifrakturCook`, not a fallback, before baking). At runtime there is no font/fontconfig/Pango — librsvg rasterises the committed paths. `test_midway_osd.py` asserts the asset is an SVG and rasterises through the osd image path to solid `#B40000` at `0.5` alpha.
