...
- `midway-genmon` — verified Midway session status for xfce4-genmon-plugin. It consumes `bin/midway-status`, which combines the local `session`/`__Host-session` expiry with Midway's live `/api/session-status` result without rewriting the cookie jar; the shared result is cached for 60 seconds while plugin 7 polls every 30 seconds. Green `MW` means a server-verified session with at least 8 hours remaining, yellow means verified with at least 2 but less than 8 hours remaining, and red means rejected, expired, absent, or verified with less than 2 hours remaining. Magenta `? MW` means the network/API check was unavailable, so only the untrusted local expiry is known. The tooltip distinguishes all states. `MIDWAY_STATUS_COMMAND` and `MIDWAY_NOW` make rendering deterministic in `bash xwindow/test_midway_genmon.sh`; the checker itself is covered by `bash bin/test_midway_status.sh`.
