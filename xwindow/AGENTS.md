...
`xmonad.symlink/lib/XMonadConfig/Hooks.hs` owns focused-window and OSD restacking, lock-screen Hangul OSD restacking, root-property event subscription, Zoom fullscreen correction, late-property floating/focus correction for the Zoom join popup, offscreen rescue, dynamic Zoom-video following, and EWMH fullscreen advertisement. `xmonad.hs` keeps start-up, event, and log hook order explicit.
...
- **Lock-screen OSD stacking**: During initialization, XMonad adds `PropertyChangeMask` to its existing root event mask so `_XMONAD_SCREEN_LOCKED` updates reach the event hooks. When that property becomes active, and after any window maps while locked, `raiseOsdOnLockHook` re-raises every window whose resource class is `osd`. This covers Hangul, Midway, Zoom, battery, volume, brightness, and audio-device OSDs regardless of whether the lock state, lock window, or OSD maps first. OSD input regions remain empty, so the overlays do not intercept lock-screen input.

## Window title tag (`windowTags`)

A title tag covers the **right half of every visible ghostty window**. Its text is left-aligned so the leading session name remains visible when a long application title is clipped. ghostty runs with `gtk-titlebar = false` and the panel has no task list, so the window title had nowhere to show. `zmx-select` stores the selected session in the UTF-8 `_ZMX_SESSION` property on Ghostty's `$WINDOWID`; `windowTags` prepends `[<session>]` to the current OSC title unless it is already present. The property keeps the session visible while a foreground TUI replaces `_NET_WM_NAME`, and after detach it remains set to the previous session while the picker displays `zmx`; it is cleared only on the initial picker before any attachment. The focused tag uses LEGO orange (`#F8BB3D`); inactive tags retain a visible `#C7CBD1` label and `#69717F` border on `#1d1d1d`.
...
- **DPI scaling is applied once**: `xmonad.tag.fontSize` uses Xft point units, so it is not multiplied manually. The current `Xft.dpi` is included explicitly in the font pattern, while the pixel-based tag height scales from its 96-DPI base of `24`. Manually scaling both the point size and the height previously doubled the font scale after a metric refresh. Hard floors of `11` points and `24` pixels prevent stale resources such as the former `4.5`/`9` settings from shrinking the tag. Width is always derived from the current client geometry.
- **Live metric refresh repaints synchronously**: `TagMetricsState` is loaded at startup, after RandR rescreen events, and whenever the root window's `RESOURCE_MANAGER` property changes. Each refresh immediately resizes and repaints visible tags, so they cannot remain at the 96-DPI defaults until a later focus or layout event. This lets `xrdb -merge` update tags without restarting xmonad; keys are `xmonad.tag.{fontSize,height}`.
- **Lock-screen suppression**: `bin/xmonad-lock-state` subscribes to GNOME Flashback's `org.gnome.ScreenSaver.ActiveChanged` signal and mirrors the state into the root-window `_XMONAD_SCREEN_LOCKED` property. `windowTags` destroys all title overlays while that property is active and recreates them on unlock, preventing override-redirect tags from appearing over the lock screen. XMonad starts the bridge at startup; a runtime `flock` keeps restarts from creating duplicate monitors. Test: `bash xwindow/test_xmonad_lock_state.sh`.
...
- Super+W/E/R → left/middle/right monitor by screen x-position, unless a work Dell or Samsung display is active
...
- `display_on_all_monitors(text, duration, style, resource_name="osd", follow_monitor_changes=True)` — show on every monitor; resource class remains `osd`, while the resource name identifies a specific overlay
...
- **Topology following by default**: every `display_on_all_monitors(...)` call subscribes to RandR screen/CRTC/output changes and rebuilds every OSD window from the current active-monitor geometry. Persistent Hangul and Midway indicators and one-shot battery and Zoom overlays therefore survive lid close/open and monitor connection changes. Callers can explicitly pass `follow_monitor_changes=False` only when a static snapshot is required.
- **Follow loop drains the internal event queue before selecting, and rebuilds only on RandR events (two python-xlib gotchas)**: the loop must call `d.pending_events()` and process any queued events *before* blocking on `select(d.fileno())`. python-xlib buffers events in an internal queue, and the `d.sync()` inside `create_windows()`/`destroy_windows()` does a non-blocking socket read — so a RandR event arriving mid-rebuild lands in that queue and leaves the fd **not readable**. Selecting on the raw fd alone then blocks forever with the event stranded. Because one monitor change emits several events back-to-back (Output/Crtc/Screen), the event carrying the *final* geometry is the one most often swallowed, so the OSD sticks on an intermediate/old layout — observed as a single mis-placed Midway window with its child at 0% CPU blocked in `select` after a DisplayLink topology change. **Second gotcha**: the loop must rebuild ONLY for RandR events, not for every drained event. Mapping our own OSD windows (they carry `ExposureMask`) makes the server queue Expose events, which `create_windows()`'s `d.sync()` then pulls into the internal queue. Rebuilding on any drained event turns our own Expose into a trigger and the OSD destroys/recreates itself ~15 times a second — visible flicker, worst on the large centred meeting/zoom overlay. Only RandR extension input is selected, so any non-core event (`type >= X.LASTEvent`) is a topology change; core events like Expose (type 12) are drained and ignored. Fix: `if not d.pending_events(): select(...)`, then drain the queue setting `topology_changed` only on `type >= X.LASTEvent`, and rebuild only when that flag is set.
- **Topology regression test**: `python3 xwindow/test_osd.py` covers three cases: (1) an fd-readable RandR event rebuilds windows for the current monitors; (2) a RandR event that is only in python-xlib's internal queue (fd never readable) still drives a rebuild — the guard for the stranded-event bug; (3) a non-RandR (Expose) event does NOT rebuild — the guard for the self-Expose flicker. Cases 2 and 3 each fail against their respective pre-fix loops.
- **Anchoring**: `anchor_y` (`top|center|bottom`) + `offset_y_frac`, mirrored for `anchor_x` + `offset_x_frac`. Physical `offset_x_mm`/`offset_y_mm` values stack on top; `sibling_offset_mm(reference, gap)` places a right-anchored box immediately left of a millimetre-sized reference. `HANGUL_SLOT_*` constants are the shared top-right geometry used by the Midway sibling.
- **Image backend**: `image_file` paints a pre-outlined SVG through librsvg instead of laying out text. The vector is scaled at the target monitor's native pixel dimensions and `fill_alpha` remains the opacity control. Midway uses the committed `osd/assets/mw.svg`, avoiding a runtime font dependency.
...
`bin/midway-osd.py` restores the persistent, click-through MW overlay on every monitor while the shared checker reports an unusable Midway session (`invalid` with a non-AEA reason, `expired`, or `missing`). AEA-only `invalid` reasons keep the overlay hidden and remain visible only in the genmon tooltip and authentication guard. `valid` hides it. An explicit `unknown` network/API state preserves the last definite state rather than creating a false transition. The long-lived `systemd.user.services.midway-osd` unit has the same graphical-session lifecycle and restart policy as the other OSD daemons.

The daemon consumes `bin/midway-status` directly but filters out any `reason` beginning with `aea` (a prefix match, not a fixed set) from the visual state, so a genuine Midway failure still shows: a local cookie that is missing or expired, or a server `not authenticated`. `just mwinit` continues to treat the AEA results as invalid. It never parses the cookie or runs authentication. Cookie and status-cache directory watches re-evaluate after login, cache invalidation, or a genmon refresh. A one-shot timer fires when the Midway session expires even if no file changes, and a 60-second safety poll detects a server-side revocation when neither file changes. Its renderer follows RandR topology changes. A separate five-second display-health poll respawns an exited child while the Midway status remains invalid. The renderer is a fresh subprocess rather than an `os.fork()` child because Gio may create threads, and Python 3.14 warns that forking a multithreaded process can deadlock. Restoring the overlay after suspend or resume therefore does not require a valid-to-invalid authentication transition.

The restored Cinzel Decorative MW vector is `osd/assets/mw.svg`, painted in LEGO Bright Red `#B40000` at 50% alpha. Its 82×42 mm box sits 2 mm left of the 60×70 mm Hangul slot and is vertically centred within it. `midway-osd --once` shows it without reading credentials, `midway-osd --render-png PATH` renders an offline preview, and `python3 xwindow/test_midway_osd.py` covers status parsing, transitions, geometry, asset selection, resource identity, and offline rendering.
...
- `midway-genmon` — Midway session plus AEA cookie and posture status for xfce4-genmon-plugin. It consumes `bin/midway-status`, which combines local credential expiration times, `mcscli is-valid posture`, and Midway's live `/api/session-status` result without rewriting the cookie jar. The AEA checks remain separate because valid device posture does not renew the cookie consumed by CR. The shared result is cached for 60 seconds while plugin 7 polls every 30 seconds. The panel label shows the coloured dot, `MW`, and the Midway time remaining in compact form (`MW 7h32m`, or `MW 2d3h` past a day) via `format_remaining_compact`; the hover tooltip keeps the fuller `Midway remaining: 7h 32m`. Only the `<txt>` label is always visible — the `<tool>` block renders only on mouseover, which is why the remaining time (long present in the tooltip) needed adding to `<txt>` to show on the panel itself. The indicator colour follows only the Midway session's remaining time: green at 8 hours or more, yellow from 2 to 8 hours, and red below 2 hours or for a definite Midway rejection, expiry, or absence. AEA expiry, absence, or rejected posture is reported in the tooltip without changing that colour. During an AEA-only failure the shared checker does not call the Midway API, so the colour band follows the still-future local Midway timestamp while the tooltip identifies the AEA problem. Magenta `? MW` means the network/API check was unavailable, so only the untrusted local expiration time is known. The persistent `midway-osd` daemon consumes the same checker but likewise ignores AEA-only reasons; `just mwinit` still treats them as invalid. `MIDWAY_STATUS_COMMAND` and `MIDWAY_NOW` make rendering deterministic in `bash xwindow/test_midway_genmon.sh`; the checker itself is covered by `bash bin/test_midway_status.sh`.
...
Multi-monitor configurations vary by location. After monitor connection changes, `rescreenHook` with `hideNSPWorkspace` swaps NSP off visible screens. When no work display is detected, Super+W/E/R map to the leftmost/middle/rightmost monitor by screen **x-position** (`positionalScreenWorkspace`), so W→left, E→middle, R→right regardless of RandR output order or output names. This is deliberately position-based rather than output-name- or EDID-based, because dock outputs are unstable: a DisplayLink dock names them `DVI-I-*` (evdi virtual outputs, needing DisplayLinkManager), while a DP-Alt/MST dock names them `DP-1-6-8` etc. (native GPU outputs via DisplayPort MST, no DisplayLink), and two same-vendor panels (e.g. dual Lenovo) can't be told apart by EDID anyway. With the laptop panel placed rightmost, R = laptop. If an active external display has the two-byte EDID manufacturer ID for Dell (`DEL`) or Samsung (`SAM`), the work profile activates instead: W=Dell, E=Samsung, and R=the internal `eDP-*` laptop panel. Super+Shift+W/E/R uses the same resolution to move the focused window to the corresponding monitor. Work targets resolve active RandR outputs to XMonad screen rectangles on each keypress instead of relying on mutable screen IDs; a disconnected work target is a no-op. DisplayLink adds a provider suffix to RandR output names (`DVI-I-1-1`) that is absent from the `/sys/class/drm` connector name (`DVI-I-1`), so EDID lookup tries both names. xfce4-panel bottom bar: 48px, using `avoidStruts`.
