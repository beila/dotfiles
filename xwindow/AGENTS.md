...
`bin/midway-osd.py` restores the persistent, click-through MW overlay on every monitor while the shared checker reports an unusable Midway session, AEA cookie, or AEA posture (`invalid`, `expired`, or `missing`). `valid` hides it. An explicit `unknown` network/API state preserves the last definite state rather than creating a false transition. The long-lived `systemd.user.services.midway-osd` unit has the same graphical-session lifecycle and restart policy as the other OSD daemons.

The daemon consumes `bin/midway-status` directly, so its Midway and AEA validity semantics are identical to genmon and `just mwinit`; it never parses the cookie or runs authentication. Cookie and status-cache directory watches re-evaluate after login, cache invalidation, or a genmon refresh. A one-shot timer fires when the first credential expires even if no file changes, and a 60-second safety poll detects a server-side revocation when neither file changes. Its renderer follows RandR topology changes. A separate five-second display-health poll respawns an exited child while the status remains invalid. The renderer is a fresh subprocess rather than an `os.fork()` child because Gio may create threads, and Python 3.14 warns that forking a multithreaded process can deadlock. Restoring the overlay after suspend or resume therefore does not require a valid-to-invalid authentication transition.
...
- `midway-genmon` — verified Midway session plus AEA cookie and posture status for xfce4-genmon-plugin. It consumes `bin/midway-status`, which combines local credential expiration times, `mcscli is-valid posture`, and Midway's live `/api/session-status` result without rewriting the cookie jar. The AEA checks remain separate because valid device posture does not renew the cookie consumed by CR. The shared result is cached for 60 seconds while plugin 7 polls every 30 seconds. Green `MW` means AEA is live and the server-verified Midway session has at least 8 hours remaining. Yellow means AEA is live and Midway has at least 2 but less than 8 hours remaining. Red means either credential is rejected, expired, absent, or Midway has less than 2 hours remaining. The AEA cookie's normal short lifetime does not use the Midway warning bands; its exact expiration and remaining time appear in the tooltip, then the indicator turns red when it expires. Magenta `? MW` means the network/API check was unavailable, so only the untrusted local expiration time is known. The persistent `midway-osd` daemon consumes the same checker, so the panel and overlay cannot disagree about a definite state. `MIDWAY_STATUS_COMMAND` and `MIDWAY_NOW` make rendering deterministic in `bash xwindow/test_midway_genmon.sh`; the checker itself is covered by `bash bin/test_midway_status.sh`.
