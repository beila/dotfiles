...
- **Floats never get decorated.** `XMonad.Layout.Decoration` only sees windows the layout placed, and the two ghostty scratchpads are floats, so they get no tag. They run zellij, whose own bar names the session. (`--window-show-tab-bar=always` on the scratchpad command line does work — each is its own ghostty instance — but the 46px strip is what the tag replaced.)
