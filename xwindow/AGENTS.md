...
- **`raiseFocused`** (logHook): calls `raiseWindow` on the focused tiled window after every focus change. Picom draws a window's shadow at that window's Z-level, so without this, the glow is hidden wherever a higher-stacked tiled neighbor overlaps the shadow zone. Raw `XRaiseWindow` bypasses the normal dock layer, so `raiseDockWindows` restores every EWMH dock after the focused client and all floats. This keeps the status panel visible even above fullscreen clients. `windowTags` then raises every title tag above the clients.
