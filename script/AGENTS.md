...
`script/bin/generalize-paths` is a standard-input-to-standard-output filter that rewrites this machine's absolute home-directory path to the standard literal `$HOME`. It also normalizes the retired `$HOME` placeholder so existing histories converge instead of reintroducing it. The filter buffers input to detect its encoding: ASCII and UTF-8 text are rewritten, while binary and other encodings pass through byte-for-byte unchanged. It is wired into jj through `[fix.tools.generalize-paths]` in `jj.configsymlink/config.toml`.
...
- GTK bookmarks are generated by Home Manager from `config.home.homeDirectory`; `gtk.css` moved alongside them so Home Manager owns the complete `~/.config/gtk-3.0` directory rather than the bootstrap symlink.
