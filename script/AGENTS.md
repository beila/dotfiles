...
`script/bin/generalize-paths` is a standard-input-to-standard-output filter that rewrites this machine's absolute home-directory path to the standard literal `$HOME`. It also normalizes the retired `USER_HOME` placeholder so existing histories converge instead of reintroducing it. The legacy token is assembled from separate string fragments in the filter and regression test so jj fix cannot rewrite its own migration rule. The filter buffers input to detect its encoding: ASCII and UTF-8 text are rewritten, while binary and other encodings pass through byte-for-byte unchanged. It is wired into jj through `[fix.tools.generalize-paths]` in `jj.configsymlink/config.toml`.

There is intentionally no reverse bootstrap filter. A global `$HOME` → absolute rewrite would also hard-code legitimate runtime `$HOME` references in tracked shell scripts and configuration, recreating machine-specific paths and repeated fix churn. Files that cannot expand `$HOME` need an explicit exception:

- `xfce4-panel.xml` and `codex.symlink/config.toml` are excluded from the fix tool and remain absolute where required.
- GTK bookmarks cannot expand environment variables, so `script/bootstrap` calls `script/bin/generate-gtk-bookmarks`. The helper renders the tracked `gtk-3.0.configsymlink/bookmarks.template` into an ignored `bookmarks` file using the machine's absolute home directory. The generated file remains available through the existing `~/.config/gtk-3.0` directory symlink but never enters jj history.

Regression coverage: `bash script/test_generalize-paths.sh`.
...
Why not git's `clean`/`smudge` filters: jj treats files as raw bytes and ignores `.gitattributes filter=` directives. The one-way jj fix keeps tracked content portable without rewriting the working tree back to machine-specific paths.
