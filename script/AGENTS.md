...
**Lock-fd inheritance** (`run_without_lock`): the per-repo `flock` lives on fd 9, and bash marks neither `9>` nor `{fd}>` close-on-exec, so fd 9 survives `exec` into every child. Any child that *daemonizes* therefore holds the lock forever and wedges that repo permanently — each later run exits at `flock -n` with "already running". Observed in the wild: `bin/commit-msg` invoked `claude`, which spawned `claude __otel-server` + `otelcol-contrib` telemetry sidecars that reparent to systemd and outlive commit-msg's `timeout 30`; both kept `fd 9 -> /tmp/sync_repo_<path>.lock` and the repo stopped syncing silently. ssh `ControlMaster` mux processes from `git push` are the same hazard. Mitigation: `run_without_lock` (`"$@" 9>&-`) wraps both `commit-msg` call sites and `timeout_cmd` (hence every git/jj network call), and the `flock -n` failure path now names the holding PIDs via `fuser` so a leak is diagnosable rather than silent. To unwedge an already-stuck repo, `rm` the lock file — the orphan keeps its lock on the now-unlinked inode while the next run creates a fresh one.
