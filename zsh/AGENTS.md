...
- **Command prefixes in `LOGRUN_AUTO_SKIP_COMMANDS`** → no rewrite. Entries are tokenized multi-word prefixes checked after alias expansion, for commands that must avoid the wrapper's PTY/pipeline rather than merely discard their log afterward. Private/work config appends domain-specific entries.
