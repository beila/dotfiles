...
- `zmx-history.zsh` — only active when `$ZMX_SESSION` is set. Registers a `zshexit` hook that synchronously runs `bin/zmx-history snapshot <session>` (bounded by a five-second `timeout`) while the zmx socket still exists, so a normal shell exit preserves output produced since the daemon's last poll. The helper's snapshot lock serializes this with daemon captures so an older concurrent snapshot cannot overwrite the final one.
