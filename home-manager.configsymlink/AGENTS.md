...
- Home Manager owns `~/.config/gtk-3.0/{bookmarks,gtk.css}`. GTK bookmarks require absolute `file://` paths and do not expand `$HOME`, so `gnome.nix` generates them from `config.home.homeDirectory`; keeping both files here avoids a parent-directory symlink conflict with the old `gtk-3.0.configsymlink`.
