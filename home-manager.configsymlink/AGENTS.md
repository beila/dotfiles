...
Vivaldi enables `proprietaryCodecs` so Nix links the compatible `libffmpeg.so` into the package. The plain package relies on Vivaldi's timed runtime downloader and failed before startup when the codec was not downloaded. It also passes `--disable-setuid-sandbox` because an immutable Nix-store helper cannot be root-owned mode `4755`; `system-deps.sh` installs `vivaldi.apparmor`, which grants `userns` only to Nix-store `vivaldi-bin` paths so Chromium's user-namespace sandbox remains enabled. A final package wrapper prefixes `${pkgs.ibus}/lib/gtk-3.0` to `GTK_PATH`, ensuring GTK3 Vivaldi can load `im-ibus.so` even when a long-lived desktop launcher still has the pre-activation environment.
...
- IBus integration (`pkgs.ibus` for ABI-matched GTK3/GTK4 IM modules + `~/.config/environment.d/30-ibus.conf` for `GTK_IM_MODULE` etc.). `GTK_PATH` includes both `${pkgs.ibus}/lib/gtk-3.0` for GTK3 Vivaldi and `${pkgs.ibus}/lib/gtk-4.0` for GTK4 Ghostty; a GTK4-only path leaves Vivaldi unable to load `im-ibus.so`, so its engine-internal `Shift+Space` Hangul toggle never reaches the IBus Hangul engine.
