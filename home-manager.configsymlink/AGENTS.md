...
- `jejuhallasan-ttf` — single `fetchurl` of one ttf from `google/fonts` (SIL OFL 1.1). Avoids `pkgs.google-fonts` (2.3 GB). Used by `hangul-osd`.
- `greatvibes-ttf` — single `fetchurl` of the Great Vibes ttf from `google/fonts` (SIL OFL 1.1); an ornate swash/script display face used by `midway-osd` for the decorative `MW` capitals.

The `hangul-osd` `writeShellScriptBin` wrapper exports `GI_TYPELIB_PATH` (Pango / PangoCairo / cairo / IBus / harfbuzz typelibs from the nix store + the gobject-introspection wrapper for cairo's `cairo-1.0.typelib`, which Pango pulls in transitively) and `HANGUL_OSD_FONT_FILE` before exec'ing the inner `writePython3Bin` impl. The `midway-osd` wrapper is the same shape: it exports `GI_TYPELIB_PATH` (Pango / PangoCairo / harfbuzz + gobject-introspection wrapper — no IBus needed), `MIDWAY_OSD_FONT_FILE` (the `greatvibes-ttf` swash face), and `MIDWAY_GENMON` (the store path of `xwindow/bin/midway-genmon`, whose `--status` mode is the daemon's only Midway state source — no cookie parsing in the daemon, no `mwinit`, no network). Both env vars are required — see `xwindow/AGENTS.md` for the Pango/fontconfig rationale. The `hangul-osd`, `zoom-osd`, and `midway-osd` systemd units use `${config.home.path}/bin/...`, not the stable profile symlink, so a changed embedded Python derivation (or, for midway-osd, the store-path parser it points at) changes `ExecStart` and Home Manager restarts the daemon.
