...
Packages, the unfree predicate (albert, Sublime Merge, Vivaldi, and 8 nvim plugins reclassified upstream — see "Flake update watchdog" in `script/AGENTS.md`), the `copyq` daemon, and `dotfiles.schedule.jobs` declarations (see `schedule.nix`). `rclone` supports the one-way, explicitly invoked `bin/dropbox-get` downloader; there is no Dropbox daemon, timer, or background mount. Anki is nixGL-wrapped so its Qt/OpenGL initialization works when launched from the desktop environment on non-NixOS hosts. Home Manager also generates Albert-visible `HojinAnki`, `InsukAnki`, `HarenAnki`, and `IrunAnki` desktop entries with isolated bases under `$XDG_DATA_HOME/Anki2-{Hojin,Insuk,Haren,Irun}`; the package's default `Anki` entry and `$XDG_DATA_HOME/Anki2` base remain unchanged. Sublime Merge supplies the `smerge` command used by jj's default merge editor. The installed `jj` package is a serialized wrapper around `pkgs.jujutsu`; see `script/AGENTS.md`.
...
- `osd` — local Python package built from `xwindow/osd/` (Cairo + XShape primitives). Used by `battery-osd`, `zoom-osd`, `hangul-osd`, and `midway-osd`; its windows have empty XShape input regions so pointer input passes through. Also exposes `sibling_offset_mm` + `HANGUL_SLOT_*` constants for placing one mm-sized OSD a fixed physical distance beside another.
...
- OSD glyph assets live in `xwindow/osd/assets/*.svg` — pre-outlined vector files generated once from their source fonts by that dir's `generate.py`. hangul-osd and midway-osd paint them via librsvg with no font at runtime, so no font is fetched in `home.nix` anymore (the former JejuHallasan / Great Vibes fetchurls were only needed by the old runtime font-render path).

The `hangul-osd` `writeShellScriptBin` wrapper exports `GI_TYPELIB_PATH` (IBus + Rsvg + GdkPixbuf + harfbuzz typelibs from the nix store + the gobject-introspection wrapper for cairo's `cairo-1.0.typelib`) and `HANGUL_OSD_IMAGE` (the packaged `hangul.svg`) before exec'ing the inner `writePython3Bin` impl. The `midway-osd` wrapper is the same shape: it exports `GI_TYPELIB_PATH` (Rsvg + GdkPixbuf + harfbuzz + gobject-introspection wrapper — no IBus needed), `MIDWAY_OSD_IMAGE` (the packaged `mw.svg`), and `MIDWAY_GENMON` (the store path of `xwindow/bin/midway-genmon`, whose `--status` mode is the daemon's only Midway state source — no cookie parsing in the daemon, no `mwinit`, no network). Both OSDs paint a pre-outlined vector SVG via librsvg — no font, fontconfig, or Pango at runtime; see `xwindow/AGENTS.md` for the asset-authoring rationale. The `hangul-osd`, `zoom-osd`, and `midway-osd` systemd units use `${config.home.path}/bin/...`, not the stable profile symlink, so a changed embedded Python derivation (or, for midway-osd, the store-path parser it points at) changes `ExecStart` and Home Manager restarts the daemon.
...
- Declares `random-lockscreen` via `dotfiles.schedule.jobs` plus `hangul-osd`, `zoom-osd`, and `midway-osd` as `systemd.user.services.*` units (`PartOf=graphical-session.target`, `Restart=on-failure`, `RestartSec=5`). All require the graphical session; hangul-osd additionally requires the panel-private interface on the IBus private message bus, and midway-osd watches the local Midway cookie via `midway-genmon --status` (a `Gio.FileMonitor` inotify watch plus a one-shot timer scheduled to the exact expiry, with a slow safety-net poll — no network, no `mwinit`).
...
Systemd-backed jobs are timer-driven oneshots with `X-RestartIfChanged=false`. Home Manager can reload a changed unit immediately, but it does not stop and restart a job already in progress; that run finishes under its original definition and the next timer trigger uses the new one. This also prevents a long `sync-repos` run from blocking an unrelated `home-manager switch`.
