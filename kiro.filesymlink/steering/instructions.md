...
5. **Repo-root `tmp/` for scratch work** — never write temporary files (debug scripts, reproductions, captured output, manual test fixtures) under `/tmp/` or `~/.tmp/`; create them under `<repo-root>/tmp/` instead. The global gitignore covers `/tmp/`, so these files stay untracked. Clean up the directory when the work is done.
