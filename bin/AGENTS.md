...
`zmx-history` — long-running snapshot daemon started by the Home Manager `zmx-history.service`. zmx's own `<session>.log` is only internal diagnostics, while actual scrollback lives in the session process, so the daemon persists rendered `zmx history` output under `${ZMX_HISTORY_DIR:-${XDG_STATE_HOME:-~/.local/state}/zmx/history}`. It snapshots all sessions at startup, then polls every `$ZMX_HISTORY_SCAN_SECONDS` (default 5s), discovers sessions from zmx's reported socket directory without connecting to them, and uses each session's cheap diagnostic-log `(mtime,size)` signature as an activity signal. Snapshots are capped at `$ZMX_HISTORY_MAX_BYTES` (default 5 MiB), staged in `$XDG_RUNTIME_DIR`, compared with `cmp`, and replaced only when rendered history changed; temporary files are flushed before rename and the containing directory is synced afterward. The state directory is mode `0700` and snapshot/name files are `0600`. Thus idle sessions produce no persistent writes, a forced reboot loses at most one scan interval, and an interrupted write leaves the prior complete snapshot. Companion CLI actions `path` / `has` / `show` / `list` are consumed by `zmx-select`; hashed filename suffixes avoid collisions after sanitising session names. Test: `bash bin/test_zmx_history.sh`.
