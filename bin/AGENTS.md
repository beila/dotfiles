...
`zmx-select` — fzf picker over zmx sessions, aliased to `z` (`zsh/utility.zsh`). It shows the sorted union of running sessions and stopped sessions with saved snapshots; there is no preset-name file or automatic `main` fallback. With no entries, the empty picker remains available for creating a typed name. Enter attaches, starting a stopped session or creating the typed name (`zmx attach` creates on demand); Ctrl-N forces a new session with the typed name (auto-suffixes `-2`, `-3`… if taken); Ctrl-D forgets the highlighted stopped session by deleting its saved snapshot, ignores running sessions, and exits instead of creating a replacement when it removes the final entry; Ctrl-/ cycles the preview through horizontal (`right:60%:follow`), vertical (`down,50%`), and hidden layouts, matching `_gh`; Ctrl-C exits. Preview shows the session's last 500 scrollback lines then streams live output (`zmx history … | tail`, then `exec zmx tail`); stopped sessions fall back to the last `zmx-history` snapshot when one exists.

`zmx-history` — long-running snapshot daemon started by the Home Manager `zmx-history.service`. zmx's own `<session>.log` is only internal diagnostics, while actual scrollback lives in the session process, so the daemon persists rendered `zmx history` output under `${ZMX_HISTORY_DIR:-${XDG_STATE_HOME:-~/.local/state}/zmx/history}`. It snapshots all sessions at startup, then polls every `$ZMX_HISTORY_SCAN_SECONDS` (default 5s), discovers sessions from zmx's reported socket directory without connecting to them, and uses each session's cheap diagnostic-log `(mtime,size)` signature as an activity signal. `zsh/zmx-history.zsh` also calls `zmx-history snapshot <session>` synchronously from `zshexit`, before the socket disappears, so a normal zsh exit includes output produced since the last poll. All captures and removals share `.snapshot.lock`, preventing an older concurrent daemon capture from overwriting the final snapshot and serializing deletion with in-flight captures. Snapshots are capped at `$ZMX_HISTORY_MAX_BYTES` (default 5 MiB), staged in `$XDG_RUNTIME_DIR`, compared with `cmp`, and replaced only when rendered history changed; temporary files are flushed before rename and the containing directory is synced afterward. The state directory is mode `0700` and snapshot/name files are `0600`. Thus idle sessions produce no persistent writes, a forced reboot loses at most one scan interval, a normal zsh exit captures final output, and an interrupted write leaves the prior complete snapshot. Companion CLI actions `snapshot` / `path` / `has` / `show` / `remove` / `list` are used by the hook and `zmx-select`; `remove` deletes both the snapshot and name index, and hashed filename suffixes avoid collisions after sanitising session names. Test: `bash bin/test_zmx_history.sh`.

Each row is `name  <cwd>`; a stopped row is bare unless a persisted snapshot exists, in which case it is marked `(saved)`. The directory is the shell's **live** cwd read from `/proc/<pid>/cwd` and `$HOME`-shortened to `~` — `zmx list` only reports `start_dir`, which stays at the creation directory no matter how much you `cd` (verified: a session `cd`'d to `/tmp` still lists `start_dir=~/.dotfiles`). The column is left blank when `/proc` can't answer; a stale directory would be worse than none. `(attached)` marks sessions with `clients > 0`, i.e. already open in another window. pid isn't displayed — it's only read to resolve the cwd.

The picker writes the window title itself: `zmx` while the list is up, `[<session>]` right before `attach`. The session's shell only re-emits its own OSC title at its next prompt, so attaching to an already-running session would otherwise leave the window — and the xmonad corner tag that reads it — labelled with whatever was there before. Both writes go to `/dev/tty`, never stdout, so they can't leak into a caller's pipeline.

Detaching is ctrl-\ (zmx's own per-client binding), which returns you to the picker because the loop below resumes when `attach` exits. `zmx detach` is deliberately NOT wired to anything: it drops *every* client of the session, and a CLI-side per-client detach is impossible in principle — all clients mirror one PTY, so code running inside the session cannot tell which window's keystrokes it is serving. Only the client, which reads your terminal, knows.

The picker **loops**: after `zmx attach` returns — detach (ctrl-\\) or the session's shell exiting — it rebuilds the list and reopens with the cursor back on the session just left, via `--bind "start:pos(N)"` (N is the row index from the parallel `$order` array). `attach` is therefore not `exec`'d. Ctrl-C / Esc in the picker is the only way out (exit 130).

New sessions are created as `zmx attach <name> zsh -l`, matching ghostty's `command = zsh -l`. Without the explicit command zmx spawns a login `$SHELL`, which is bash on this account (LDAP, no `chsh`) — verified: a bare `zmx attach` session runs `-bash`, with the command it runs `zsh -l`. The command only applies at creation; sessions already running bash keep bash until they're killed.
