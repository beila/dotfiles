...
- **`say-en`** — piper-tts. Picks a voice from a pool (`en_GB-alba-medium`, `en_US-amy-medium`, `en_US-ryan-high`, `en_GB-cori-high`, `en_US-lessac-medium`) by hashing the caller key (see below) and downloads *only the selected* model on demand (falls back to alba if the download fails, so an offline box still speaks). `$PIPER_MODEL` overrides the whole pool/hash mechanism.
- **`say-ko`** — edge-tts (requires internet). Picks from a pool (`ko-KR-SunHiNeural`, `ko-KR-InJoonNeural`, `ko-KR-HyunsuMultilingualNeural`) by the same caller-key hash. Default rate `+50%`, override with `$EDGE_TTS_RATE`; `$EDGE_TTS_VOICE` overrides the pool/hash.
- **Random-but-stable voice per caller** — both backends choose `index = cksum(key) mod poolsize`; deterministic, stateless. The **key** is `$SAY_VOICE_KEY` if set, else `$PPID`. An unidentified caller (key empty or `1` — a `setsid`'d process that passed no key) uses the pool's first entry (SunHi / alba), i.e. the historical default. `say` resolves the key from *its own* `$PPID` (the real caller) and exports `SAY_VOICE_KEY` before spawning the backend — necessary because `say`'s `setsid` would otherwise reset the backend's `$PPID` to 1. The three detaching callers set `$SAY_VOICE_KEY` themselves (their own `say`/backend runs under `setsid`, so PPID is 1): `claude-stop-tts` / `claude-notification-tts` pass the Claude `session_id` (stable per session), `mcp-tts` passes its server `$$` (stable per server lifetime). `kiro-response` `exec`s `say-ko` directly, so the backend's `$PPID` fallback covers it with no change. Test: `bash bin/test_say_voice.sh`.
