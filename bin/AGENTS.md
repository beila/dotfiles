...
- **`say`** — routes by content language. Hangul (U+AC00–U+D7A3) → `say-ko`, otherwise → `say-en`. Accepts text as args or stdin. **Preempts** still-playing audio: spawns the backend under `setsid` (PID==PGID), records the PGID at `${SAY_STATE_FILE:-${XDG_RUNTIME_DIR:-/tmp}/say.pgid}`, and `TERM`s the previous PGID on the next call. A `TERM`/`INT`/`HUP` trap forwards to the backend session so external preemption (e.g. by `mcp-tts`) tears down the whole audio pipeline instead of orphaning aplay. Bypass with `SAY_NO_PREEMPT=1`. **Say-mute mode**: when `${SAY_MUTE_FILE:-${XDG_RUNTIME_DIR:-/tmp}/say-mute}` exists, `say` drops the request silently (still drains stdin to avoid SIGPIPE in callers) instead of speaking — other audio is unaffected. The flag is toggled by `xwindow/bin/volume-osd toggle`'s 3-state cycle (normal → audio-muted → say-muted → normal); entering say-mute also `TERM`s the active say PGID so the currently-playing utterance stops immediately. **Auto-mute during meetings**: after the say-mute check, `say` queries `pw-dump | jq` for any `Stream/Input/Audio` whose `application.process.binary` or `application.name` matches `$SAY_MEETING_APP_REGEX` (default `zoom|teams|meet|webex|slack|chime|discord`) and silently no-ops if found. Zoom keeps its capture stream live even with in-app mute (its mute is drop-on-send, not capture-side cork), so this fires whether or not you're muted in the call. Bypass with `SAY_NO_MEETING_CHECK=1`; auto-skipped on hosts where `pw-dump`/`jq` aren't on PATH. Test: `bash bin/test_say_preempt.sh`.
