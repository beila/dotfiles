...
- **Common backend CLI** — `say-en`, `say-ko`, and `say-es` accept `--voice N` (one-based exact pool entry), `--speed PERCENT` (`25`, `+25%`, or `-10%`), `--list-voices`, and text as arguments or stdin. Omit `--voice` for the existing deterministic caller-key selection. A CLI voice number takes precedence over the backend-specific model/voice environment override. For Edge TTS, CLI speed takes precedence over `$EDGE_TTS_RATE`; for Piper it is converted to inverse `--length-scale` so positive percentages are faster. Options precede text; use `--` when text itself begins with `-`.
- **`say-en`** — piper-tts. Pool: `en_GB-alba-medium`, `en_US-amy-medium`, `en_US-ryan-high`, `en_GB-cori-high`, `en_US-lessac-medium`. Downloads only the selected model on demand and falls back to alba if that download fails. `$PIPER_MODEL` overrides automatic pool selection. No `--speed` keeps Piper's normal rate.
