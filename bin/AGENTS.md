...
- **`say-voice.sh`** — the _single place_ the caller→voice mapping lives (sourced by `say`, `say-ko`, `say-en`; never run directly). `say_pick_index <poolsize>` returns `sha256(key) mod poolsize` — **sha256, not `cksum`/CRC32**: CRC32 is linear over GF(2) and clusters structured keys (sequential PIDs, prefix-sharing session ids), whereas sha256 stays uniform for whatever opaque key the caller passes, and the caller needs no knowledge of the pool size. Deterministic and stateless, so a given key always maps to the same voice.
  - **Key contract** (`say_resolve_key`): `$SAY_VOICE_KEY` is an opaque, arbitrary identity string. **Unset** → fall back to `$PPID` (stable for interactive/direct callers; only an orphan `PPID==1` stays unidentified). **Set but empty** → the caller has no identity → **unidentified** → the pool's first entry (SunHi / alba), the historical default. Any non-empty value (including `"1"`) is honoured verbatim — the setsid/PPID reality is _not_ leaked into the key meaning.
