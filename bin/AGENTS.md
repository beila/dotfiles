...
- **PTY layer for color** (all paths in `--auto`): the inner command is wrapped in `script -qefc` so its stdout is a real PTY. Color-aware tools (`eza`, `ls --color=auto`, `git`, `grep --color=auto`) see `isatty(stdout) == true` and emit color and multi-column layout, which logrun then ferries through tee to the user's terminal. Applies to `-c` shell mode, positional argv, AND the `--no-zshrc` fast path — the latter previously skipped the wrapper, which is why `logrun --auto --no-zshrc -- eza` rendered as a single uncolored column. `script` is in util-linux (universal on Linux); if absent, falls back to bare exec and color is lost. The PTY's canonical mode adds `\r` before `\n`; the side awk strips it from the on-disk log so files match the non-PTY representation. Original command name is captured pre-rewrite so log filenames stay readable (e.g. `log-echo-hi-...txt`, not `log-script-qefc-zsh-ic-...`).
- **fd 4 isolation** (`--auto` only): the inner command is invoked with `4>&-`, closing the awk side-channel fd before exec. Without this, `zsh -ic` helpers (gitstatus daemon, zsh-async workers) inherit fd 4 and keep the awk pipe writer open after the user's command exits — they get reparented to systemd but retain the fd, so `wait $awk_pid` idles for several seconds before they self-terminate. tee's own fd 4 (used by `tee /dev/fd/4`) is unaffected because the redirection only applies to `"$@"`.
- **Wallclock timer detached stdio**: the wallclock-timer subshell starts with `exec </dev/null >/dev/null 2>&1` so its `sleep $auto_seconds` child can't hold the parent's stdin/stdout/stderr. `kill $auto_timer_pid` only signals the bash subshell, not the inner sleep — without the redirection, the orphaned sleep would block any downstream pipe reader (e.g. `logrun … | cat`) for the full $auto_seconds even after a 2-second command finished.
