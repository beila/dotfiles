...
`zmx-select` — fzf picker over zmx sessions, aliased to `z` (`zsh/utility.zsh`). Lists ad-hoc running or saved sessions first (sorted, since associative-array key order is a hash order), then the preset names from `bin/zmx-sessions` (override via `$ZMX_SESSIONS_FILE`) in file order — so that part of the layout doesn't move as sessions start and stop. Enter attaches, starting the session if it's stopped (`zmx attach` creates on demand, so no separate create step); Ctrl-N forces a new session with the typed name (auto-suffixes `-2`, `-3`… if taken); Ctrl-C exits. Preview shows the session's last 500 scrollback lines then streams live output (`zmx history … | tail`, then `exec zmx tail`) with `--preview-window=follow`; stopped sessions fall back to the last `zmx-history` snapshot when one exists. Falls back to attaching a default session (CLI arg, `$ZMX_DEFAULT_SESSION`, or `main`) only when there are no presets, running sessions, or saved snapshots.

`zmx-history` — long-running snapshot daemon started by the Home Manager `zmx-history.service`. zmx's own `<session>.log` is only internal diagnostics, while actual scrollback lives in the session process, so the daemon persists rendered `zmx history` output under `${ZMX_HISTORY_DIR:-${XDG_STATE_HOME:-~/.local/state}/zmx/history}`. It snapshots all sessions at startup, then polls every `$ZMX_HISTORY_SCAN_SECONDS` (default 5s), discovers sessions from zmx's reported socket directory without connecting to them, and uses each session's cheap diagnostic-log `(mtime,size)` signature as an activity signal. `zsh/zmx-history.zsh` also calls `zmx-history snapshot <session>` synchronously from `zshexit`, before the socket disappears, so a normal zsh exit includes output produced since the last poll. All captures share `.snapshot.lock`, preventing an older concurrent daemon capture from overwriting the final snapshot. Snapshots are capped at `$ZMX_HISTORY_MAX_BYTES` (default 5 MiB), staged in `$XDG_RUNTIME_DIR`, compared with `cmp`, and replaced only when rendered history changed; temporary files are flushed before rename and the containing directory is synced afterward. The state directory is mode `0700` and snapshot/name files are `0600`. Thus idle sessions produce no persistent writes, a forced reboot loses at most one scan interval, a normal zsh exit captures final output, and an interrupted write leaves the prior complete snapshot. Companion CLI actions `snapshot` / `path` / `has` / `show` / `list` are used by the hook and `zmx-select`; hashed filename suffixes avoid collisions after sanitising session names. Test: `bash bin/test_zmx_history.sh`.
