...
Running `z` **inside** a session that the picker attached detaches instead of nesting a second picker: `$XDG_RUNTIME_DIR/zmx-select/<session>` holds the waiting picker's PID while `attach` runs (removed after, plus an EXIT trap), and the session-side check `exec zmx detach`es only when that PID is still alive. The session's shell can't infer this from its process tree — its parent is the zmx server, not the client — hence the marker. A stale marker or a hand-attached session (no marker) falls through to a normal picker. Note `zmx detach` drops *all* clients of the session, not just this one.
