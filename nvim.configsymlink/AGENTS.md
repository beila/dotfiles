...
- **JJ workspace switch** (`lua/jj-workspace-picker.lua`) — `<leader>gw` / `<C-g><C-w>` opens an fzf-lua picker of `jj workspace list` rows (shows the workspace name; the workspace root rides in a hidden tab field). The preview runs that workspace's own `jj log` via `jj -R <root> log`. Selecting a workspace rebases the launcher window's file from its current workspace root onto the chosen workspace's root (via `jj workspace root` + `vim.fs.relpath`) and points every window in the current tab at that file, `lcd`-ing each window into the target root so relative pickers, LSP root detection, and the jj statusline follow. Selecting the current workspace, a file living outside its workspace, or a path absent from the target workspace is reported and aborts without touching windows. Regression coverage: `test_jj_workspace_picker.lua`.
