...
- **JJ workspace switch** (`lua/jj-workspace-picker.lua`) — `<leader>B` / `<C-g>b` opens an fzf-lua picker of `jj workspace list` rows (shows the workspace name; the workspace root rides in a hidden tab field), mirroring zsh's `ctrl-g B` workspace widget (`_jbb`). The preview runs that workspace's own `jj log` via `jj -R <root> log`. The current (launcher) workspace is flagged with a trailing 🟢 emoji after its name, so every name stays left-aligned in the same column. Selecting a workspace rebases the launcher window's file from its current workspace root onto the chosen workspace's root (via `jj workspace root` + `vim.fs.relpath`) and points every window in the current tab at that file, `lcd`-ing each window into the target root so relative pickers, LSP root detection, and the jj statusline follow. Selecting the current workspace, a file living outside its workspace, or a path absent from the target workspace is reported and aborts without touching windows. Root resolution probes the launcher file's directory, then the window cwd, then the global cwd (so a window whose cwd sits outside the repo still resolves via its open file), and a failure names the reason (jj-not-on-PATH vs jj stderr vs no directory) rather than a bare "not inside a JJ workspace". `Ctrl-B` inside either dialog toggles to the other — the bookmark (branch) picker `M.bookmarks()` (`jj bookmark list`, boundary preview, Enter copies the bookmark name to the `"`/`0` registers) — matching the zsh `_jbb` ↔ `_jb` toggle. `<leader>B` also shadows nvim-dap's `toggle_breakpoint`; the `<C-g>b` binding is collision-free. Regression coverage: `test_jj_workspace_picker.lua`.
