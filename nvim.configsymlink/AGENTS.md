...
- **CodeDiff** (`vimrcs/codediff.lua`, `lua/jj-diff-picker.lua`) — CodeDiff 2.67 is pinned in `home-manager.configsymlink/nvim.nix` for gutter-sign support. `<leader>gh` / `<C-g><C-h>` opens the JJ revision picker; `<leader>gH` / `<C-g><C-l>` limits it to revisions touching the current file. Enter compares one selected revision with its parent; fzf multi-selection compares the sole parent of the oldest selected revision with the newest selected descendant, including any revisions between them. Selections spanning unrelated branches or a merge root are rejected. CodeDiff consumes Git commit objects, so JJ mode requires a colocated Git/JJ repository; outside JJ, the mappings retain the fzf-lua Git fallbacks.
