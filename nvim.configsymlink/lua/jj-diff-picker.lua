...
local function resolve_range(root, selected)
    ...
    local roots = run_jj(root, {
        "log", "--no-graph", "-r", "roots(" .. selected_revset .. ")", "-T", template,
    })
    local heads = run_jj(root, {
        "log", "--no-graph", "-r", "heads(" .. selected_revset .. ")", "-T", template,
    })
    ...
    local parents = run_jj(root, {
        "log", "--no-graph", "-r", "parents(" .. roots[1] .. ")", "-T", template,
    })
    if not parents then
        return nil
    end
...
local function preview_command(path)
    ...
    table.insert(commands,
        [[jj --ignore-working-copy --quiet diff --color=always -r "$id"]] .. path_arg)
...
local function log_command(path)
    local args = {
        "jj", "--quiet", "log", "--no-pager", "--color=always",
        "-T", path and "fzf_oneline_author" or "fzf_oneline",
    }
    if path then
